...
export default function ContentRowMovies(props){
    return(
        <div className="row">
        {/*Es importante que lo que se haga sea todo dentro de una sola etiqueta ya que React lo lee de esta forma*/} 
    ...
    customProp: function(data){
        for(let i=0; i<data.data.length; i++){
            if(typeof data.data[i].color !== "string" || typeof data.data[i].cifra !== "number" || typeof data.data[i].titulo !== "string"){
                return new Error ("Invalid props, color and titulo must be a string and cifra must be a number")
            }
        }
    }
}
